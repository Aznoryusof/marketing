...
```{r}
test_scores = read.csv(file = "./data/testscoresdata.csv")
test_scores
```
...
- A factor is a group of variables measuring something in common
- All the variables in factor analysis has been standardised to mean == 0 and var == 1
- The total variability in the data is the sum of the variances, 6 * 1
- The eigenval values is equivalent to the number of variable that are captured by the factor
- Criterion >1 means that the group for the factor must have at least one variable
...
- Factor analysis tries to summarise the data while explaining as much variability as possible
- First, find the strongest correlations between the variables and the latent factor
- Then, find the second set of correlations and calls it the second factor
- The initial solution can lead to a strong correlation of a variable with several factors, or variables that has no strong correlations with any of the factors
- Rotating the axis can make the location of the axes if the actual data points better, allowing the factors to be more interpretable
- The different types of rotations can create factors that are correlated or uncorrelated with each other
- Oblique rotations allow for correlation, orthogonal rotations assumes factors are uncorrelated 

The eigenvalues of the correlation matrix are related to the sum of squared loadings, they tell us the variance explained by the factor
